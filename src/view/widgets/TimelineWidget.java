...
public class TimelineWidget extends AbstractWidget {
	...
	public void setValues(ArrayList<Float> values) {
		...
		float width = (this.getWidthXYGlobal()-2*this.getStrokeWeight())/getValues().size();
		
		for (int i=0; i<this.getValues().size(); i++) {
			float value = this.getValues().get(i);
			float bx = this.getCenterPointGlobal().x - this.getWidthXYGlobal()/2 + this.getStrokeWeight() + interspacing + i*width;
